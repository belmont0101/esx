	...
	while true do
		...
		if #(coords - Config.CircleZones.WeedProcessing.coords) < 1 then
			...
			if not isProcessing then
				lib.showTextUI(TranslateCap('weed_processprompt'), {
...
function ProcessWeed(xCannabis)
	...
	lib.notify({
		...
		description = TranslateCap('weed_processingstarted'),
		type = 'success'
	})
  ...
  local timeLeft = (Config.Delays.WeedProcessing * xCannabis) / 1000
	...
	while timeLeft > 0 do
		...
		if #(GetEntityCoords(playerPed) - Config.CircleZones.WeedProcessing.coords) > 4 then
			lib.notify({
				title = 'Weed Processing',
				description = TranslateCap('weed_processingtoofar'),
				type = 'error'
			})
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp then
				lib.showTextUI(TranslateCap('weed_pickupprompt'), {
					position = "right-center",
					icon = 'seedling'
				})
			end
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				ESX.TriggerServerCallback('esx_drugs:canPickUp', function(canPickUp)
					if canPickUp then
						...
						lib.notify({
							title = 'Inventory',
							description = TranslateCap('weed_inventoryfull'),
							type = 'error'
						})
					end
