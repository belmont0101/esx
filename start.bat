...
set source=C:\Users\Max\Desktop\esx\artifacts
...
:: Create isolated working directory for server runtime
set SERVER_WORKDIR=%TEMP%\FiveMServer_%RANDOM%
mkdir "%SERVER_WORKDIR%" >nul 2>&1

:: Copy server config to working directory
copy "%configFolder%%config%" "%SERVER_WORKDIR%\%config%" >nul 2>&1

:: Create symbolic link to resources folder so server can find them
mklink /D "%SERVER_WORKDIR%\resources" "%configFolder%resources" >nul 2>&1

:: Run server from isolated directory
cd /d "%SERVER_WORKDIR%"
%~1 +exec %config%

:: Cleanup working directory after server stops
cd /d "%configFolder%"
rmdir /s /q "%SERVER_WORKDIR%" >nul 2>&1
